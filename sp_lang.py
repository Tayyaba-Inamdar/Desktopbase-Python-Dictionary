...
3)two meanings are opposite only within the context of their relationship are relational antonyms''', font=("Gabriola", 15),bg='white', justify=LEFT).pack()
        ...
        def evaluate(event):
            ...
            if (a.isalpha() == True):
                res.configure(text='''Antonym of:  ''' + a + " \n" + str(dic.antonym(a)))
            else:
                tkMessageBox.showinfo("INVALID INPUT", ''' Please enter appropriate  word!!!''')

        Label(self, text="Type and hit enter...", font=("Gabriola", 20), fg="red",bg='white').pack()
        e = Entry(self, font=("Gabriola", 20))
...
class PageThree(tk.Frame):
    def __init__(self, parent, controller):
        ...
        img = Image.open('main_bg -MyDoc.png')
        self.tkimage = ImageTk.PhotoImage(img)
        myvar=tk.Label(self,image = self.tkimage)
        myvar.pack()
        myvar.place(x=0, y=0)
              
        label = tk.Label(self, text="About Self Access Langue", font=TITLE_FONT,bg='white')
        label.pack(side="top", fill="x", pady=10)
        ...
        logo = tk.Label(self,
                        text='''\t Our project, Self-access langue is an desktop application,developed by using python language with the help of active python idle which is helpful to learn grammar easily
Smart English Doc is a learning and practice grammar app designed to help improve English langue accuracy.
The app is suitable for learners it not only check the proper usage of specific words, but will also check the spelling that you use with certain words so that they will be used in the right context.
...
Smart English Doc is very useful for people who are learning the English language and can also be used as a personal tutor to help you in your study of English grammar.''',
                        font=("Gabriola", 15), justify=LEFT,bg='white').pack()
...
class PageFour(tk.Frame):
    def __init__(self, parent, controller):
        ...
        def hello():
            tkMessageBox.showinfo("Noun", '''A noun is a part of speech that names a person, place, thing, action.
...
EXAMPLE: Animals,birds,names,place,etc''')
        ...
        def hello1():
            tkMessageBox.showinfo("Pronoun", '''A pronoun is a word or phrase that may be substituted for a noun or noun phrase, which once replaced,is known as the pronouns antecedent.
...
Its because pronouns can do everything that nouns can do. A pronoun can act as a subject,direct object, indirect object, object of the preposition, and more.''')
        ...
        def hello2():
            tkMessageBox.showinfo("Adjectives", '''Adjectives are words that describe or modify other words, making your writing and speaking much more specific, and a whole lot more interesting.
    ...
    Because adjectives are used to identify or quantify individual people and unique things, they are usually positioned before the noun or pronoun that they modify. Some sentences contain multiple adjectives''')
        ...
        def hello3():
            tkMessageBox.showinfo("Tenses", '''A verb-based method used to indicate the time, and sometimes the continuation or completeness,of an action or state in relation to the time of speaking.
...
The verbs that express a state of being take a little practice to spot, but, actually, they are the most common. The most common verb is the verb to be. Below is the verb to be in the different tenses:''')
        ...
        B6 = tk.Button(self, text="Chapter 7:\nPhrases", command=hello6, font=('Gabriola', 20), height=1, width=12,
                            bg='cyan', relief="sunken")
        B6.pack()
...
long and extended are synonymous within that context.''', font=("Gabriola", 15),bg='white',justify=LEFT).pack()
        def evaluate(event):
            ...
            if (a.isalpha() == True):
                ...
                tkMessageBox.showinfo("INVALID INPUT", ''' Please enter a word!!!''')
...
Words can be synonymous when meant in certain senses, even if they are not synonymous in all of their senses.
For example, if one talks about a long time or an extended time,
long and extended are synonymous within that context.''', font=("Gabriola", 15), bg='white',
              justify=LEFT).pack()
        ...
        e.bind("<Return>", evaluate)
        e.pack()
        res = Label(self, font=("Gabriola", 15), fg="blue",bg="white",relief="solid",wraplength=900)
        res.pack()
...
class PageSeven(tk.Frame):
    def __init__(self, parent, controller):
        ...
        img = Image.open('main_bg-ABT.jpg')
        self.tkimage = ImageTk.PhotoImage(img)
        myvar=tk.Label(self,image = self.tkimage)
        myvar.pack()
        myvar.place(x=0, y=0)
...
Quality- Respectful and self-confident team leader that reassures team members of his authority within the organization.
...
She has done the coding and also implementated the GUI of our Application.
Quality- A person who loves to work out how things work
...
Quality- Understands and is committed to team objectives
...
belongs to Computer department of A.R.Kalsekar Polytechnic.
Rimsha is the team member of this group
She has done the Documentation and also supported in the GUI designing of our Application.
Quality- Has skills in understanding what's going on in the group
...
Contact:8652593993''')
        ...
        label1.pack()
        label1.place(x=425, y=200)

        img1 = Image.open('p_amn.jpg')
        self.tkimag = ImageTk.PhotoImage(img1)
        docButton = Button(self, text='ANSARI AIMAN SABA ',height=260, width=200, font=('Gabriola', 15),  relief=RAISED,
                           command=changeLabel,image = self.tkimag,compound="top")
        docButton.pack()
        docButton.place(x=200, y=125)
        
        img2 = Image.open('p_tabz.jpg')
        self.tkimg = ImageTk.PhotoImage(img2)
        docButton = Button(self, text='INAMDAR TAYYABA', height=260, width=200, font=('Gabriola', 15),  relief=RAISED,
                           command=changeLabel1,image = self.tkimg,compound="top")
        docButton.pack()
        docButton.place(x=930, y=125)

        img3 = Image.open('p_irm.jpg')
        self.tkimg3 = ImageTk.PhotoImage(img3)
        docButton = Button(self, text='FANGARI IRAM', font=('Gabriola', 15),  relief=RAISED, height=260, width=200,
                           command=changeLabel2,image = self.tkimg3,compound="top")
        docButton.pack()
        docButton.place(x=200, y=405)
        
        img4 = Image.open('p_rim.jpg')
        self.tkimg4 = ImageTk.PhotoImage(img4)
        docButton = Button(self, text='HAKIM RIMSHA', font=('Gabriola', 15),relief=RAISED,height=260, width=200,
                           command=changeLabel3,image = self.tkimg4,compound="top")
        docButton.pack()
        docButton.place(x=930, y=405)
